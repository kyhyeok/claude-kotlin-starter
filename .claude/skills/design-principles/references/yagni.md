...
YAGNI는 _자기-테스트 코드(SelfTestingCode)와 지속적 통합_ 위에서만 안전하게 작동한다. 테스트 없이 YAGNI를 적용하면 변경에 대한 안전망이 사라져 리팩토링이 막힌다. 이 프로젝트는 행위 단위 테스트(`*_specs.kt`)가 그 안전망이다 (CLAUDE.md §5).
