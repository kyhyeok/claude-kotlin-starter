...
코드로 박제 불가하므로 본 ADR이 명시 근거. 자세한 체크리스트는 `docs/git-workflow.md` §6:
...
- Require linear history (squash 정책과 정합, merge commit 자동 차단).
- Require conversation resolution before merging (미해결 리뷰 코멘트 차단).
